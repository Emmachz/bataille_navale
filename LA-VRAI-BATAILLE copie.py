...
def create_grid():
   L = [[VIDE for i in range(N)] for y in range(N)]
   return L
...
def input_ajout_bateau(flotte,nom,taille):
    posi=input("Entrer une position (une ligne(a-z) et une colonne(0-9) (exemple: 'a 4'): ")
    orientation=input("Entrer une orientation (h ou v): ")
    r=(r"^[a-j]\s[0-9]$")
    orie=(r"^[h|v]$")
...
def init_ia():
    ...
    for i in range(len(NOMS)):
        ...
        while nouveau_bateau(flotte,NOMS[i],TAILLES[i],posi,orientation)==False:
            posi=random_position()
        print("Veuillez donnez la position du bateaux %str de taille %s : " %(NOMS[i],TAILLES[i]))
        print(posi)
        print(orientation)
    ...
    while c==3:
        ...
        if pos_from_string(posi)!=False:
            ...
            if tir(pos,m,flotte)!=False:
                ...
                print("position est deja attaquer")
...
def test_fin_partie(nom,m,flotte,nb_tour):
    if flotte==[]:
        print(nom, "a perdu la bataile au bout de ", nb_tour , "tour ! MAIS PAS LA GUERRE !!!")
        return True
...
def joueur_vs_ia():
    ...
    while tour!=-1 :
        ...
        hide()
        tour+=1
        if test_fin_partie(nom,m1,flotte1,tour)==True or test_fin_partie("ordi",mia,flotteia,tour)==True:
            tour=-1
...
print()
print("Règle du jeu: Affrontez votre adversaire et tentez de détruire tous les bateaux composant sa flotte pour remporter la victoire !")
print() 
nb_de_joueur=input("Mode: 1 joueur (saisir 1) ou 2 joueurs (saisir 2): ")
